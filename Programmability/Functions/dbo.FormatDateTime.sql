...
BEGIN 
    ...
    WHEN 'MM/DD/YY hh:mm:ss' THEN 

        CONVERT(CHAR(8), @dt, 1) + ' ' +  CONVERT(CHAR(8), @dt, 8)

	WHEN 'HHMMSS' THEN
	  REPLACE(LEFT(CONVERT(VARCHAR(8), @dt, 8), 8),':', SPACE(0)) 
...
/*
declare @dt datetime
set @dt=dbo.GetLocalDATE()
select [dbo].[FormatDateTime](@dt,'MM/DD/YY hh:mm:ss')*/
GO
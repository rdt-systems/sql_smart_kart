...
begin     
 ...
 BEGIN    
  ...
  SELECT        ItemPiece.ItemID, ItemPiece.Name, ItemPiece.ModalNumber, ItemPiece.BarcodeNumber, ItemPiece.OnOrder as OnOrder, ItemPiece.OnHand, ItemPiece.Department,     
  ...
  FROM            ItemPiece Inner JOIN    
        ...
        WHERE ItemPiece.ItemType <> 2 AND ItemPiece.ItemType <> 3  AND ItemPiece.ItemType <> 9  AND (ItemPiece.StoreNo =@StoreID  or @StoreID is null) /*AND ISNULL(OnHand,0) <> 0*/
 ...
 BEGIN    
  ...
  WHERE   ItemPiece.ItemType <> 2 AND ItemPiece.ItemType <> 3  AND ItemPiece.ItemType <> 9  AND  (StoreNo = @StoreID Or @StoreID is null) /*AND ISNULL(OnHand,0) <> 0    */
 END    
...
set @Date =dbo.GetLocalDATE()    
 ...
 from (    
  SELECT        ItemPiece.ItemID, ItemPiece.Name, ItemPiece.ModalNumber, ItemPiece.BarcodeNumber, iod.OnOrder as OnOrder, dbo.GetItemOnHand(ItemPiece.itemStoreId,@Date) as OnHand, ItemPiece.Department,ItemPiece.Cost, ItemPiece.Price,     
         Store.StoreName, Store.StoreID, ItemPiece.SupplierName, ItemPiece.OnTransfer, ParentInfo.[Supplier Item Code] AS ParentCode,     
  ...
  FROM            ItemPiece inner JOIN    
		 ...
		 LEFT OUTER JOIN    
          (SELECT DISTINCT ItemID, SupplierName, StoreNo, [Supplier Item Code]    
            FROM            ItemMainAndStoreView AS ItemMainAndStoreView_1) AS ParentInfo ON ItemPiece.StoreNo = ParentInfo.StoreNo AND     
         ItemPiece.LinkNo = ParentInfo.ItemID    
		 Left Outer Join ItemOnOrder(@Date,@StoreID) iod on iod.ItemStoreId=ItemPiece.itemStoreId
        WHERE ItemPiece.ItemType <> 2 AND ItemPiece.ItemType <> 3  AND ItemPiece.ItemType <> 9  AND (ItemPiece.StoreNo =@StoreID  or @StoreID is null) /*AND ISNULL(OnHand,0) <> 0 */  
  ) as ta    
 END     
 ELSE    
 ...
 SELECT     ta.ItemID,    
  ...
  SELECT     dbo.ItemPiece.ItemID,    
     ...
     CaseQty,    
     MainDepartment    
  FROM       dbo.ItemPiece left join     
     (SELECT ItemID FROM (SELECT ItemID FROM dbo.ItemPiece    
     GROUP BY ItemID,Price) AS Items     
     GROUP BY ItemID HAVING count(*)>1) AS AllItems    
     ON AllItems.ItemID=dbo.ItemPiece.ItemID
	  left join     Store ON Store.StoreID=dbo.ItemPiece.StoreNo 
		 Left Outer Join ItemOnOrder(@Date,@StoreID) iod on iod.ItemStoreId=ItemPiece.itemStoreId   
  WHERE   ItemPiece.ItemType <> 2 AND ItemPiece.ItemType <> 3  AND ItemPiece.ItemType <> 9  AND  (StoreNo = @StoreID Or @StoreID is null) /*AND ISNULL(OnHand,0) <> 0*/) as ta    
 END    

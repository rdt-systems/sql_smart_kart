...
GO
CREATE PROCEDURE [dbo].[SP_GetItemMainAndStoreView]
(
...
PRINT 'SELECT 3'
 set @Select =' With Page as (SELECT     ROW_NUMBER() OVER (Order By DateCREATEd Asc) AS Row, 
		         dbo.ItemMainAndStoreGrid.ItemStoreID 
			FROM         dbo.ItemMainAndStoreGrid
			WHERE  (StoreNo = '''+@sStoreID+''') AND   (Status> 0) AND (MainStatus>-1)'--)
SET @Select1 ='	SELECT     Page.Row ,ItemMainAndStoreGrid.*
            FROM  ItemMainAndStoreGrid INNER JOIN Page ON ItemMainAndStoreGrid.ItemStoreID = Page.ItemStoreID  

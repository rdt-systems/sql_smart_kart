...
FROM            CustomerToGroup as Cg  WHERE CG.CustomerID = Customer.CustomerID AND CG.Status > 0
...
LEFT OUTER JOIN (SELECT        CustomerAddresses.*
FROM            CustomerAddresses
WHERE        (Status > 0) AND (AddressType = 6))AS 
                         CustomerAddresses ON Customer.MainAddressID = CustomerAddresses.CustomerAddressID

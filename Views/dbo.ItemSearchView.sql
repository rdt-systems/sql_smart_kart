...
GO
CREATE  VIEW [dbo].[ItemSearchView]
   -- with schemabinding
AS
SELECT        ItemMainView.ItemID, ItemMainView.Name, ItemMainView.ModalNumber, ItemMainView.LinkNo, ItemMainView.BarcodeNumber, ItemMainView.CustomInteger1, ItemStoreView.StoreNo, Store.StoreName, Store.StoreNumber, 
                         ...
                         ItemMainView.CostByCase, ItemMainView.CaseBarcodeNumber, ItemStoreView.OnHand, CAST(CASE WHEN ISNULL(ItemMainView.CaseQty, 1) > 0 AND (ISNULL(dbo.ItemStoreView.OnHand, 1) <> 0) 
                         THEN ISNULL(dbo.ItemStoreView.OnHand, 1) / ISNULL(ItemMainView.CaseQty, 1) ELSE dbo.ItemStoreView.OnHand END AS decimal(20, 2)) AS CsOnHand, ItemStoreView.BinLocation, ItemStoreView.Status, 
                         ...
                         ISNULL(ItemStoreView.MTDDollar, 0) AS MTD, ISNULL(ItemStoreView.MTDQty, 0) AS [MTD Pc Qty], (CASE WHEN dbo.ItemMainView.CaseQty <> 0 THEN dbo.ItemStoreView.MTDQty / dbo.ItemMainView.CaseQty END) 
                         AS [MTD Cs Qty], ISNULL(ItemStoreView.YTDDollar, 0) AS YTD, ISNULL(ItemStoreView.YTDQty, 0) AS [YTD Pc Qty], 
                         (CASE WHEN dbo.ItemMainView.CaseQty <> 0 THEN dbo.ItemStoreView.YTDQty / dbo.ItemMainView.CaseQty END) AS [YTD Cs Qty], ISNULL(ItemStoreView.PTDDollar, 0) AS PTD, ISNULL(ItemStoreView.PTDQty, 0) 
                         AS [PTD Pc Qty], ItemMainView.MatrixTableNo, (CASE WHEN dbo.ItemMainView.CaseQty <> 0 THEN dbo.ItemStoreView.PTDQty / dbo.ItemMainView.CaseQty END) AS [PTD Cs Qty], ItemStoreView.ItemNo, 
                         ...
                         = '' THEN dbo.ItemHeadSupplierView.ItemCode ELSE ParentInfo.[Supplier Item Code] END) AS ParentCode, ItemStoreView.SalePrice AS RegSPPrice, ItemStoreView.CaseSpecial AS CaseSPPrice, NULL AS RegPkgPrice, NULL 
                         AS [Reg SP Price Markup], NULL AS [Reg SP Price Margin], NULL AS [Pkg Price Markup], NULL AS [Pkg Price Margin], NULL AS [SP Markup], NULL AS [SP Margin], NULL AS SeasonName, NULL AS SeasonID, NULL 
...
FROM            DepartmentStore RIGHT OUTER JOIN
                         SupplierView INNER JOIN
                         ItemHeadSupplierView ON SupplierView.SupplierID = ItemHeadSupplierView.SupplierNo RIGHT OUTER JOIN
                         ItemStoreView INNER JOIN
                         Store ON Store.StoreID = ItemStoreView.StoreNo AND Store.Status > 0 INNER JOIN
                         ItemMainView ON ItemStoreView.ItemNo = ItemMainView.ItemID LEFT OUTER JOIN
                         Manufacturers ON ItemMainView.ManufacturerID = Manufacturers.ManufacturerID ON ItemHeadSupplierView.ItemSupplyID = ItemStoreView.MainSupplierID ON 
                         DepartmentStore.DepartmentStoreID = ItemStoreView.DepartmentID AND DepartmentStore.Status > 0 LEFT OUTER JOIN
                         SysItemTypeView ON ItemMainView.ItemType = SysItemTypeView.SystemValueNo LEFT OUTER JOIN
                         SysBarcodeTypeView ON ItemMainView.BarcodeType = SysBarcodeTypeView.SystemValueNo LEFT OUTER JOIN

...
FROM            dbo.DepartmentStore WITH (NOLOCK) RIGHT OUTER JOIN
						 ...
						 --INNER JOIN
       --                      (SELECT        StoreID, CONVERT(decimal(18, 2), OptionValue) AS Cap
       --                        FROM            dbo.SetUpValues WITH (NOLOCK)  
       --                        WHERE        (OptionID = 797) AND (StoreID <> '00000000-0000-0000-0000-000000000000')) AS Cap ON ItemStore.StoreNo = Cap.StoreID LEFT OUTER JOIN
...
WHERE        (ItemStore.Status > 0) AND (ItemMain.Status > 0)
GO